...
from . import STFTFB, Decoder, Encoder, transforms  # noqa
from .stft_fb import perfect_synthesis_window


def griffin_lim(
    mag_specgram, stft_enc, angles=None, istft_dec=None, n_iter=6, momentum=0.9
):
...
def misi(
    ...
    if angles is None:
        angles = (
            2 * math.pi * torch.rand_like(mag_specgrams, device=mag_specgrams.device)
        )
